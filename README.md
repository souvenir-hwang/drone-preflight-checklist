...
드론 기체에 QR 코드를 부착해두면, 조종자가 드론 번호를 몰라도 스캔만으로 해당 드론이
자동 고정된 점검 화면에 바로 진입할 수 있다.

- `admin-drones.html` 각 드론 행의 **QR 버튼**을 누르면 `index.html?drone=<코드>` 주소를
  인코딩한 QR 코드 이미지(PNG)가 다운로드된다. 이 이미지를 인쇄해 해당 드론에 붙이면 된다.
- `index.html`은 URL에 `?drone=` 파라미터가 있으면 해당 드론으로 선택을 고정(비활성화)하고,
  파라미터가 없으면 기존처럼 드롭다운에서 직접 선택하는 방식으로 동작한다.
- QR 이미지 생성은 브라우저에서 `qrcode`(jsDelivr CDN) 라이브러리로 처리하며 서버를 거치지 않는다.
